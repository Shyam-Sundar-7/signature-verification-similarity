...
from inference1 import Inference_prediction1
from inference2 import Inference_prediction2
...
def main():
    ...
    if img1 is not None and img2 is not None:
        if st.button("Compute Similarity Between Two images"):
            # Extract features from the images
            inference1 = Inference_prediction1("models/best_model1.ckpt")
            x1=inference1.predict(img1,img2)
            inference2 = Inference_prediction2("models/best_model2.ckpt")
            x2=inference2.predict(img1,img2)

            # Display the similarity score
            st.markdown("The Similarity score between the two images ")
            st.markdown("The Larger the score, Higher the Similarity")
